...
class Category(Base):
    ...
    items = relationship("latestItem", cascade="all, delete-orphan", 
                         backref="category")
...
class latestItem(Base):
    ...
    @property
    def serialize(self):
        """Return object data in easily serializeable format because of JSON
        Here we can control what be will send across"""
        return {
           'name': self.name,
